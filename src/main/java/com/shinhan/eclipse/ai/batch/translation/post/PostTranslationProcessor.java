...
public class PostTranslationProcessor implements ItemProcessor<IpoScore, IpoScore> {
    ...
    @Override
    public IpoScore process(IpoScore score) {
        List<Long> ids = parseIds(score.getPostTopNewsIds());
        if (ids.isEmpty()) return null;

        IpoNews news1 = newsRepository.findById(ids.get(0)).orElse(null);
        ...
        try {
            ...
            if (contentKo1 != null) newsRepository.updateContentKo(news1.getId(), contentKo1);
            if (contentKo2 != null) newsRepository.updateContentKo(news2.getId(), contentKo2);

            log.info("PostTranslationJob 요약·전문 번역 완료: ipoId={}, ticker={}", score.getIpoId(), score.getTicker());
            score.updatePostSummary(summary);
            return score;
        } catch (Exception e) {
            log.warn("PostTranslationJob 요약 실패: ipoId={}, error={}", score.getIpoId(), e.getMessage());
            return null;
        }
    }
